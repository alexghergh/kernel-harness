...
This is the top-level setup step that problem launchers call before handing control to
the solver agent. It dispatches to the registered problem source for reference loading
and baseline measurement, then renders the solver-facing docs and archives the contract.
"""
...
from kernel_bench_experiment_agents.problem_source import DEFAULT_PROBLEM_SOURCE, get_problem_source
from kernel_bench_experiment_agents.runtime.common import emit_json, normalize_tool_name
from kernel_bench_experiment_agents.runtime.project import archive_problem_dir, build_problem_root, write_json
from kernel_bench_experiment_agents.workspace.archive import (
    archive_problem_contract_dir,
    write_archive_problem_manifest,
)
...
def command_prepare_problem_workspace(args: argparse.Namespace) -> None:
    ...
    problem_source_name = (getattr(args, "problem_source", None) or DEFAULT_PROBLEM_SOURCE).strip()
    problem_source = get_problem_source(problem_source_name)

    reference_payload = problem_source.prepare_problem(
        level=args.level,
        ...
        problem_dir=getattr(args, "problem_dir", None),
    )
    ...
    metadata = build_problem_metadata(
        ...
        problem_name=reference_payload.problem_name,
        problem_source=problem_source,
        problem_dir=reference_payload.problem_metadata.get("problem_dir"),
        hardware=hardware,
        hardware_name=args.hardware_name,
    ...
    provenance_payload = build_archive_provenance(
        problem_source_name=problem_source.name,
        reference_payload=reference_payload,
        kernelbench_root_path=str(Path(args.kernelbench_root).expanduser().resolve()) if args.kernelbench_root else None,
        timings_dir=str(Path(args.timings_dir).expanduser().resolve()) if args.timings_dir else None,
        eager_baseline_file=reference_payload.baseline.extras.get("eager_baseline_file"),
        compile_baseline_file=reference_payload.baseline.extras.get("compile_baseline_file"),
        problem_dir=reference_payload.problem_metadata.get("problem_dir"),
    )
    ...
    write_contract_bundle(
        ...
        metadata=metadata,
        baseline=reference_payload.baseline,
        hardware_payload=hardware_payload,
        problem_source=problem_source,
        reference_source=reference_payload.reference_source,
    )
