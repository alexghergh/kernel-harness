...
def render_workspace_agents_md(*, contract: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        *[f"- `{path}`" for path in contract["edits"]],
        ...
        "- when spawning `runner` or `profiler`, do not request a full-history fork; spawn the named helper with only the task prompt",
...
def render_initial_prompt(*, contract: dict[str, Any], baseline: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "WHEN you want a measured evaluation, spawn the `runner` helper if available. WHEN you want profiling or profile interpretation, spawn the `profiler` helper if available. Do not request a full-history fork for these helpers; spawn the named helper with only the task prompt. Fall back to direct MCP calls only when helper spawning is unavailable.",
