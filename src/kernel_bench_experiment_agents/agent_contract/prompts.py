...
def render_workspace_agents_md(*, contract: dict[str, Any], problem_source: ProblemSource) -> str:
    ...
    lines = [
        ...
        *[f"- `{path}`" for path in contract["edits"]],
        ...
        "- WHEN helper agents are available and you want a measured evaluation, spawn `runner` so the main context does not get polluted by run output",
        "- WHEN helper agents are available and you want Nsight Compute output or profile interpretation, spawn `profiler` so the main context stays focused on design decisions",
...
def render_codex_helper_instructions(*, spec: Any, problem_source: ProblemSource) -> str:
...
def render_claude_helper_body(*, spec: Any, problem_source: ProblemSource) -> str:
    tool_list = ", ".join(f"`{name}`" for name in spec.mcp_tools)
    read_list = _helper_read_list(spec, problem_source)
    return (
