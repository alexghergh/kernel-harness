...
_CATALOG = (
    HardwareSpec(
        ...
        guidance=(
            ...
            "TMA (Tensor Memory Accelerator): host-side `cuTensorMapEncodeTiled` plus device-side `cp.async.bulk.tensor.{2d,3d}.shared::cluster.global` with `cp.async.bulk.commit_group` / `cp.async.bulk.wait_group`. Async bulk global→shared copies with hardware-managed addressing; faster and lower overhead than `cp.async`.",
            "wgmma (warp-group MMA): `wgmma.mma_async.sync.aligned.m64nNk16.f32.f16.f16` family. One warp-group (4 warps) issues one mma covering a 64xN tile; higher tensor-core throughput than `mma.sync.aligned.m16n8k16`. Requires arch `sm_90a` (not `sm_90`).",
            "Thread-block clusters + DSMEM: launch with `__cluster_dims__(x,y,z)`. Blocks within a cluster can read/write each other's shared memory through the distributed-shared address space, enabling larger effective tiles without proportionally more SMEM per block.",
            "`mbarrier` PTX (`mbarrier.init.shared::cta`, `mbarrier.arrive`, `mbarrier.try_wait`) is the canonical way to overlap TMA loads with `wgmma` compute in a producer/consumer pipeline.",
            ...
            "Default build flag for the above is `-arch=sm_90a` (the `a` variant). Without `a`, `wgmma` and several TMA forms are unavailable.",
            "Web access is allowed for `docs.nvidia.com` only; using it to look up the exact PTX encoding for these intrinsics is cheaper than re-deriving it from scratch.",
        ),
        doc_urls=(
