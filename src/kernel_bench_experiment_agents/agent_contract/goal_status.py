...
from kernel_bench_experiment_agents.agent_contract.policy import ALLOWED_WEB_DOMAINS
    ...
    if resolved:
        ...
        recommended_actions.extend(
            [
                ...
                f"WHEN the next optimization idea depends on NVIDIA-specific behavior, use hosted web search on the allowed domains ({', '.join(ALLOWED_WEB_DOMAINS)}) for topics like tensor cores, WMMA, async copy/pipelining, occupancy, bank conflicts, and memory hierarchy limits. Other domains are blocked by policy.",
