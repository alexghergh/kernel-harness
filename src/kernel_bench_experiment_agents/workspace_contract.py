...
def render_workspace_agents_md(*, contract: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        *[f"- `{path}`" for path in contract["edits"]],
        ...
        "",
        f"- if your runtime exposes generated helper agents, use them proactively and by default for isolated evaluation or profiling work: {helper_names}",
        "- prefer delegating measured evaluation to `runner` and profiling to `profiler` so the main context stays focused on implementation choices",
...
def render_initial_prompt(*, contract: dict[str, Any], baseline: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "If your runtime exposes helper agents `runner` and `profiler`, use them by default for measured evaluations and Nsight Compute work so those outputs stay out of the main context. Fall back to direct MCP calls only when helper spawning is unavailable.",
